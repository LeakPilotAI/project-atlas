...
def _trend(value:Any)->str:
    ...
    if text not in {"UP","DOWN","FLAT","UNKNOWN","OFF"}:raise ValueError(f"invalid htf_trend: {value!r}")
    return text

def _context(path:Path)->list[HistoricalMarketContext]:
    out=[]
    for row in _rows(path):
        missing=[k for k in ("timestamp","open_interest_usd","volume_24h_usd") if row.get(k) in (None,"")]
        if missing:raise ValueError(f"historical context missing fields: {', '.join(missing)}")
        trend=row.get("htf_trend");legacy=row.get("htf_regime_aligned")
        if trend in (None,"") and legacy in (None,""):raise ValueError("historical context missing fields: htf_trend")
        out.append(HistoricalMarketContext(str(row["timestamp"]),float(row["open_interest_usd"]),float(row["volume_24h_usd"]),_bool(legacy) if legacy not in (None,"") else None,_trend(trend) if trend not in (None,"") else None))
    return out
...
def _funding(path:Path)->list[dict[str,Any]]:
    out=[]
    for row in _rows(path):
        time_value=row.get("time");rate=row.get("funding_rate",row.get("fundingRate"))
...
def import_dataset(*,symbol:str,timeframe:str,candles_path:Path,funding_path:Path,context_path:Path,output_path:Path,candles_source:str,funding_source:str,oi_source:str,rolling_volume_source:str,htf_context_source:str,notes:str="")->tuple[Path,Path]:
    ...
    dataset=write_canonical_csv(rows,Path(output_path));raw_hashes={"candles":_sha(Path(candles_path)),"funding":_sha(Path(funding_path)),"context":_sha(Path(context_path))}
    ...
    manifest_path=dataset.with_suffix(dataset.suffix+".manifest.json");write_manifest(manifest,manifest_path);return dataset,manifest_path
...
def main(argv:list[str]|None=None)->int:
    ...
    p.add_argument("--symbol",required=True);p.add_argument("--timeframe",required=True);p.add_argument("--candles",type=Path,required=True);p.add_argument("--funding",type=Path,required=True);p.add_argument("--context",type=Path,required=True);p.add_argument("--output",type=Path,required=True)
    ...
    a=p.parse_args(argv);dataset,manifest=import_dataset(symbol=a.symbol,timeframe=a.timeframe,candles_path=a.candles,funding_path=a.funding,context_path=a.context,output_path=a.output,candles_source=a.candles_source,funding_source=a.funding_source,oi_source=a.oi_source,rolling_volume_source=a.rolling_volume_source,htf_context_source=a.htf_context_source,notes=a.notes)
