...
def _json_http(body: Any, status_code: int = 200) -> JSONResponse:
    try:
        ...
        payload = {"ok": False, "title": "ATLAS EDGE DIAGNOSTICS", "error": f"serialize: {type(e).__name__}: {str(e)[:180]}", "live_capital_allowed": False, "disclaimer": "Diagnostics failed to serialize. Journal was not rewritten."}
...
async def edge_endpoint() -> JSONResponse:
    try:
        ...
        body = {"ok": False, "title": "ATLAS EDGE DIAGNOSTICS", "error": f"{type(e).__name__}: {str(e)[:240]}", "live_capital_allowed": False, "baseline": {"n": 0, "winrate": 0.0, "expectancy": 0.0, "total_r": 0.0}, "malformed_count": 0, "section_errors": [str(e)[:240]], "policy_guard_activations": 0, "diagnostics_healthy": False, "disclaimer": "Diagnostics failed to fully build. Journal was not rewritten."}
...
async def challenger_lab_endpoint() -> JSONResponse:
    try:
        ...
        body = {"ok": False, "title": "ATLAS V6 CHALLENGER LAB", "error": f"{type(e).__name__}: {str(e)[:240]}", "production_strategy_modified": False, "live_capital_allowed": False, "automatic_real_money_execution": False}
    return _json_http(body, 200)

@router.get("/challengers/prospective")
async def challenger_prospective_endpoint() -> JSONResponse:
    """Durable forward-only V6 cohort. Syncs research membership only; never execution."""
    try:
        from app.services.challenger_prospective import prospective_report
        body = prospective_report()
    except Exception as e:
        body = {"ok": False, "title": "ATLAS V6 PROSPECTIVE CHALLENGER COHORT", "error": f"{type(e).__name__}: {str(e)[:240]}", "production_strategy_modified": False, "live_capital_allowed": False, "automatic_real_money_execution": False}
    return _json_http(body, 200)
