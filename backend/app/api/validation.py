...
def _json_http(body: Any, status_code: int = 200) -> JSONResponse:
    ...
    except Exception as e: payload = {"ok": False, "title": "ATLAS EDGE DIAGNOSTICS", "error": f"serialize: {type(e).__name__}: {str(e)[:180]}", "live_capital_allowed": False, "disclaimer": "Diagnostics failed to serialize. Journal was not rewritten."}
...
async def challenger_exit_replay_endpoint() -> JSONResponse:
    """Chronological persisted-mark replay only; never changes production exits."""
    try:
