...
MIN_CLOSED=100; MIN_QUALIFYING_SNAPSHOTS=3; MIN_SEPARATION_HOURS=6.0; MIN_NEW_CLOSES=1
...
def stability_report(*,path:Path=HISTORY_PATH)->Dict[str,Any]:
    ...
    for name in names:
        ...
        independent=[]; rejected_unchanged=0; rejected_time=0
        for r,p in qualifying:
            ts=_parse(r.get("timestamp")); closed=int(p.get("closed") or 0)
            if ts is None:continue
            if not independent:
                independent.append((ts,r,p));continue
            prior_ts,_,prior_p=independent[-1]; prior_closed=int(prior_p.get("closed") or 0)
            if closed-prior_closed<MIN_NEW_CLOSES:
                rejected_unchanged+=1;continue
            if (ts-prior_ts).total_seconds()<MIN_SEPARATION_HOURS*3600:
                rejected_time+=1;continue
            independent.append((ts,r,p))
        recent=independent[-MIN_QUALIFYING_SNAPSHOTS:]; sustained=len(recent)>=MIN_QUALIFYING_SNAPSHOTS
        ...
        candidates[name]={"qualifying_snapshot_count":len(independent),"raw_qualifying_snapshot_count":len(qualifying),"required_qualifying_snapshots":MIN_QUALIFYING_SNAPSHOTS,"minimum_separation_hours":MIN_SEPARATION_HOURS,"minimum_new_closes":MIN_NEW_CLOSES,"rejected_unchanged_evidence":rejected_unchanged,"rejected_temporal_dependence":rejected_time,"independence_established":bool(sustained),"sustained_positive_expectancy":no_expectancy_reversal,"sustained_positive_lower_ci95":no_ci_reversal,"contemporaneous_risk_stable":risk_complete,"risk_rule":"candidate max_drawdown_r <= contemporaneous baseline max_drawdown_r for every qualifying review snapshot","human_review_eligible":bool(sustained and no_expectancy_reversal and no_ci_reversal and risk_complete),"production_promoted":False}
    eligible=[n for n,v in candidates.items() if v["human_review_eligible"]]
    return {"ok":True,"title":"ATLAS V6 MULTI-SNAPSHOT PROSPECTIVE STABILITY","mode":"APPEND_ONLY_FORWARD_EVIDENCE_ONLY","snapshot_count":len(rows),"candidates":candidates,"human_review_eligible":eligible,"history_rewritten":False,"retrospective_substitution":False,"automatic_promotion":False,"production_strategy_modified":False,"trading_readiness":"NOT_READY","live_capital_allowed":False,"automatic_real_money_execution":False,"note":"Human review requires repeated positive forward evidence with materially new challenger closes, >=6h separation, and contemporaneous drawdown no worse than baseline. Repeated refreshes over unchanged evidence cannot manufacture stability."}
