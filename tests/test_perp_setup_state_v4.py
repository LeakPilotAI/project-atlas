...
def test_long_state_progression():
    ...
    assert classify_setup_state(mark=102.0, entered=True, **kw).state is PerpSetupState.TP1_HIT
    assert classify_setup_state(mark=104.0, entered=True, **kw).state is PerpSetupState.TP2_HIT
...
def test_short_state_progression():
    ...
    assert classify_setup_state(mark=98.0, entered=True, **kw).state is PerpSetupState.TP1_HIT
    assert classify_setup_state(mark=96.0, entered=True, **kw).state is PerpSetupState.TP2_HIT


def test_tp_is_not_reported_before_entry():
    long_kw = dict(side=Side.LONG, l1=99.0, l2=98.0, l3=97.0, stop=96.0, tp1=102.0, tp2=104.0)
    short_kw = dict(side=Side.SHORT, l1=101.0, l2=102.0, l3=103.0, stop=104.0, tp1=98.0, tp2=96.0)
    assert classify_setup_state(mark=104.0, **long_kw).state is PerpSetupState.WAIT
    assert classify_setup_state(mark=96.0, **short_kw).state is PerpSetupState.WAIT
