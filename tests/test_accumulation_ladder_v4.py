...
def test_accumulate_arms_patient_frozen_levels_below_current_quote(tmp_path):
    ...
    assert [x["price"] for x in state["levels"]] == [97.0, 95.0, 92.0, 88.0]
    assert [x["pct_below_anchor"] for x in state["levels"]] == [3.0, 5.0, 8.0, 12.0]


def test_adobe_255_95_anchor_puts_l1_near_248(tmp_path):
    store = AccumulationLadderStore(tmp_path / "ladder.json")
    now = datetime(2026, 9, 14, 14, 30, tzinfo=timezone.utc)
    store.sync(board(255.95), now=now)
    state = store.get("ADBE")
    assert state["levels"][0]["price"] == 248.2715
...
def test_live_quote_can_arm_and_trigger(tmp_path):
    ...
    hits = store.sync(board(96.9, quality="LIVE"), now=now + timedelta(seconds=30))
...
def test_robinhood_ask_is_used_for_buy_limit_crossing(tmp_path):
    ...
    assert store.sync(board(96.9, trigger=97.1), now=now + timedelta(seconds=30)) == []
    hits = store.sync(board(96.8, trigger=96.99), now=now + timedelta(seconds=60))
    assert [x.level for x in hits] == ["L1"]
...
def test_overlay_exposes_next_level_and_distance(tmp_path):
    ...
    assert status["distance_to_next_level_pct"] == 3.0
...
def test_each_level_alerts_once_after_successful_delivery(tmp_path):
    ...
    now = datetime(2026, 9, 14, 14, 30, tzinfo=timezone.utc)
    store.sync(board(100.0), now=now)
    first = store.sync(board(96.9), now=now + timedelta(minutes=1))
    assert [x.level for x in first] == ["L1"]
    assert store.mark_delivered("ADBE", first[0].cycle_id, "L1", now=now + timedelta(minutes=1)) is True
    assert store.sync(board(96.0), now=now + timedelta(minutes=2)) == []
    second = store.sync(board(94.9), now=now + timedelta(minutes=3))
...
def test_gap_down_emits_every_newly_crossed_level(tmp_path):
    ...
    now = datetime(2026, 9, 14, 14, 30, tzinfo=timezone.utc)
    store.sync(board(100.0), now=now)
    hits = store.sync(board(91.0), now=now + timedelta(minutes=1))
...
def test_stale_quote_cannot_arm_or_trigger(tmp_path):
    ...
    hits = store.sync(board(80.0, quality="STALE"), now=now + timedelta(minutes=2))
    assert hits == []
    assert all(not x["hit"] for x in store.get("ADBE")["levels"])


def test_v2_ladder_is_hidden_then_rearmed_on_fresh_quote(tmp_path):
    path = tmp_path / "ladder.json"
    path.write_text(
        '{"symbols":{"ADBE":{"schema_version":2,"active":true,"cycle_id":"v2","anchor_price":255.95,"levels":[{"level":"L1","price":252.1107,"hit":false}]}}}',
        encoding="utf-8",
    )
    store = AccumulationLadderStore(path)
    pre = store.overlay(board(255.95, quality="REFERENCE"))[0]
    assert pre["accumulation_ladder"] is None
    assert pre["accumulation_status"]["state"] == "WAITING_FOR_FRESH_QUOTE"
    now = datetime(2026, 9, 14, 14, 30, tzinfo=timezone.utc)
    store.sync(board(255.95, quality="LIVE"), now=now)
    state = store.get("ADBE")
    assert state["schema_version"] == SCHEMA_VERSION
    assert state["anchor_price"] == 255.95
    assert state["levels"][0]["price"] == 248.2715
